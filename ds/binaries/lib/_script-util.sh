...
display_java_not_found_error() {
  echo "Please set OPENDJ_JAVA_HOME to the root of a Java 17 (or higher) installation"
...
set_client_classpath() {
  set_opendj_classpath opendj-bootstrap-client.jar
}

# Configure the appropriate CLASSPATH for server, using OpenDJ logger.
set_server_classpath() {
  set_opendj_classpath opendj-bootstrap-server.jar
}
...
if [ -n "$TERM" ] && [ -z "$COLUMNS" ] && [ -x /usr/bin/tput -o -x /bin/tput ]; then
