...
rem Portions Copyright 2011-2025 Ping Identity Corporation.
...
call "%INSTALL_ROOT%\lib\setcp.bat" %INSTALL_ROOT%\lib\opendj-bootstrap-client.jar
...
FOR /F "delims=" %%i IN ("%INSTALL_ROOT%")  DO set "INSTALL_ROOT=%%~dpnxi"
FOR /F "delims=" %%i IN ("%INSTANCE_ROOT%") DO set "INSTANCE_ROOT=%%~dpnxi"
call "%INSTALL_ROOT%\lib\setcp.bat" %INSTALL_ROOT%\lib\opendj-bootstrap-server.jar
set "CLASSPATH=%INSTANCE_ROOT%\classes;%CLASSPATH%"
FOR %%x in ("%INSTANCE_ROOT%\extlib\*.jar") DO call "%INSTALL_ROOT%\lib\setcp.bat" %%x
set "SET_CLASSPATH_DONE=true"
goto scriptBegin
...
echo You must specify the path to a valid Java 17 or higher version.
echo The procedure to follow is to set the environment variable OPENDJ_JAVA_HOME
echo to the root of a valid Java 17 installation.
...
rem read the provided property from config/java.properties and store it in the PROPERTY_VALUE variable
:readProperty
set PROPERTY_VALUE=
