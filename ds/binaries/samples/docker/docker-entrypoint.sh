        ...
        for cluster in ${DS_CLUSTER_TOPOLOGY//,/ }; do
            ...
            ## MCS (GKE multi-cluster Services)
                ...
                if [ "${clusterIdentifier}" == "${podService}" ]; then
                    ...
                    DS_ADVERTISED_LISTEN_ADDRESS="${podPrefix}-0.${cluster}.${clusterIdentifier}.${podDomain#*.}"
                else
                    additional="${podPrefix}-0.${cluster}.${clusterIdentifier}.${podDomain#*.}:8989"
                    newBootstrapServers="${newBootstrapServers},${additional}"
                fi
            ## KubeDNS
            else
                ...
                if [ "${clusterIdentifier}" == "${podService}" ]; then
                    ...
                    additional="${podPrefix}-0.${clusterIdentifier}.${podDomain#*.}:8989"
                    newBootstrapServers="${newBootstrapServers},${additional}"
                fi
            fi
        done
...
upgradeDataAndRebuildUntrustedIndexes() {
    ...
    echo "Upgrading configuration and data..."
     ./upgrade --dataOnly --acceptLicense --force --ignoreErrors --no-prompt --onlySetUntrusted

    # Rebuild any untrusted indexes.
    for baseDn in "${BASE_DNS[@]}"; do
        echo "Rebuilding untrusted indexes for base DN \"${baseDn}\"..."
        rebuild-index --offline --noPropertiesFile --rebuildUntrusted --baseDn "${baseDn}" > /dev/null
    done
}
...
setUserPasswordInLdifFile() {
    ...
    if [[ ! -s "$ldifFile" ]]; then
        echo -n "setUserPasswordInLdifFile: Source ldif to modify [$ldifFile] is empty or missing... "
        if [[ -s "${ldifFile}.bak" ]]; then
            # A backup file from the previous run exists
            cp -v "${ldifFile}.bak" "${ldifFile}"
            echo "setUserPasswordInLdifFile: Restored from ${ldifFile}.bak"
        elif [[ -s "${ldifFile}.old" ]]; then
            # A backup file from the DS LDIF backend exists
            cp -v "${ldifFile}.old" "${ldifFile}"
            echo "setUserPasswordInLdifFile: Restored from ${ldifFile}.old"
        else
            echo "setUserPasswordInLdifFile: Could not locate a backup ldif file!"
            exit 1
        fi
    fi

    echo "setUserPasswordInLdifFile: Updating the \"${dn}\" password"
    ...
    ldifmodify --outputLdif "${ldifFile}.tmp" "${ldifFile}" << EOF
dn: ${dn}
changetype: modify
replace: userPassword
userPassword: ${enc_pwd}
EOF
    if [[ ! -s "${ldifFile}.tmp" ]]; then
        echo "setUserPasswordInLdifFile: WARNING: REFUSING to update ${ldifFile} because ${ldifFile}.tmp was empty"
    else
        mv -v "${ldifFile}.tmp" "${ldifFile}"
    fi
}
