...
The PingDS server sample dashboard is a
[Grafana](https://grafana.com/) dashboard that graphs data
stored in [Prometheus](https://prometheus.io/):

* By default, PingDS servers publish Prometheus-format metrics at `/metrics/prometheus/0.0.4`.
* Prometheus regularly pulls metrics from each PingDS server.
* Grafana graphs metrics queried from Prometheus.

_**Disclaimer:**
This sample is provided on an "as is" basis, without warranty of any kind, to 
the fullest extent permitted by law. Ping Identity does not warrant or guarantee 
the individual success developers may have in implementing the code on their 
development platforms or in production configurations. Ping Identity does not 
...
relating to this sample. Ping Identity disclaims all warranties, expressed or 
...
Ping Identity shall not be liable for any direct, indirect or consequential 
...
* PingDS servers, assigning a password to the default monitor user, `uid=Monitor`.
  Prometheus will access the metrics with username `monitor` and password `password`.
* Prometheus, configured to access metrics from the PingDS servers.
...
This section demonstrates how to set up a pair of PingDS replicas to monitor.

This section assumes you already know how to set up and use PingDS servers.
For background information and additional details see the
[PingDS documentation](${doc.base.url}/).

### Set Up a Pair of PingDS Replicas to Monitor

Prepare a couple of DS/RS server replicas to monitor,
as shown in the following example script.

The script assumes you have downloaded the PingDS server distribution,
...
### Run Load Against PingDS Replicas
...
When configuring Prometheus, make sure that it pulls metrics from your PingDS servers.

If you set up a pair of PingDS replicas as described in the previous section,
...
scrape_configs:
  ...
  - job_name: 'PingDS'
    ...
    scrape_interval: 15s
    scrape_timeout: 5s
    metrics_path: '/metrics/prometheus/0.0.4'
...
* Check the Prometheus URL on a PingDS server to make sure the metrics are published.
  For example, browse <http://opendj.example.com:18080/metrics/prometheus/0.0.4>,
  authenticating with username `monitor` and password `password`.
* Check that the PingDS targets are up using the Prometheus targets page.
...
* Import the sample PingDS dashboard into Grafana,
  selecting PingDS as the data source.
* (Optional) Generate more load on the PingDS replicas as described above.
...
For reference information about PingDS server metrics,
see the PingDS _[Reference](${doc.base.url}/reference/#chap-monitoring)_.
